function loadHTML(myDivId) {
	htmlText = replaceHtmlDynamicProps(htmlText), document.getElementById(myDivId).innerHTML = htmlText;
	for (var allScripts = document.getElementById(myDivId).getElementsByTagName("script"), n = 0; n < allScripts.length; n++) eval(allScripts[n].innerHTML)
}

function replaceHtmlDynamicProps(t) {
	return t = t.replace("{{iframeSrc}}", fobiBaseLocation + fobiId + "?chatPrimary=" + encodeURI(chatPrimary).replace('#', '') + "&chatSecondary=" + encodeURI(chatSecondary).replace('#', '') + "&chatBotImg=" + encodeURI(chatBotImg).replace('#', '')), t = t.replace("{{headerBackground}}", headerBackground), t = t.replace("{{headerTitleColor}}", headerTitleColor), t = t.replace("{{headerIconColor}}", headerIconColor), t = t.replace("{{buttonOffset}}", buttonOffset), t = t.replace("{{chatboxOffset}}", chatboxOffset), t = t.replace("{{chatboxHeight}}", chatboxHeight), t = t.replace("{{chatboxWidth}}", chatboxWidth), t = replaceAll(t, "{{buttonImg}}", buttonImg), t = t.replace("{{buttonBackground}}", buttonBg), t = t.replace("{{botTitle}}", botTitle), t = t.replace("{{heightMinus100}}", chatboxHeight.slice(0, -2) - 100 + 'px')
}
...
d_buttonImg = "https://app.fobi.io/icon.png", d_buttonBg = "#3e6cb4", d_popUpOpen = "";
document.addEventListener("DOMContentLoaded", function (t) {
	injectDivElement = document.getElementById(injectDiv), fobiId = injectDivElement.getAttribute("data-fobi-id"), botTitle = injectDivElement.hasAttribute("data-bot-title") ? injectDivElement.getAttribute("data-bot-title") : d_botTitle, popUpOpen = injectDivElement.getAttribute("data-pop-up-open") ? injectDivElement.getAttribute("data-pop-up-open") : d_popUpOpen, headerBackground = injectDivElement.getAttribute("data-hd-bg") ? injectDivElement.getAttribute("data-hd-bg") : d_headerBackground, headerTitleColor = injectDivElement.getAttribute("data-hd-ti-clr") ? injectDivElement.getAttribute("data-hd-ti-clr") : d_headerTitleColor, headerIconColor = injectDivElement.getAttribute("data-hd-ico-clr") ? injectDivElement.getAttribute("data-hd-ico-clr") : d_headerIconColor, chatPrimary = injectDivElement.getAttribute("data-ct-pm") ? injectDivElement.getAttribute("data-ct-pm") : d_chatPrimary, chatSecondary = injectDivElement.getAttribute("data-ct-sc") ? injectDivElement.getAttribute("data-ct-sc") : d_chatSecondary, chatBotImg = injectDivElement.getAttribute("data-ct-bot-img") ? injectDivElement.getAttribute("data-ct-bot-img") : d_chatBotImg, buttonOffset = injectDivElement.getAttribute("data-btn-offset") ? injectDivElement.getAttribute("data-btn-offset") : d_buttonOffset, chatboxHeight = injectDivElement.getAttribute("data-cb-height") ? injectDivElement.getAttribute("data-cb-height") : d_chatboxHeight, chatboxWidth = injectDivElement.getAttribute("data-cb-width") ? injectDivElement.getAttribute("data-cb-width") : d_chatboxWidth, chatboxOffset = injectDivElement.getAttribute("data-cb-offset") ? injectDivElement.getAttribute("data-cb-offset") : d_chatboxOffset, buttonImg = injectDivElement.getAttribute("data-btn-img") ? injectDivElement.getAttribute("data-btn-img") : d_buttonImg, buttonBg = injectDivElement.getAttribute("data-btn-bg") ? injectDivElement.getAttribute("data-btn-bg") : d_buttonBg, loadHTML(injectDiv), setInitPopupStatus()
});